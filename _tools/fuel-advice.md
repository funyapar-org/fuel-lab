...
layout: default
title: 燃費改善アドバイスツール
description: 車の現状を入力すると、何をどう改善すれば燃費が伸びるかを優先順位付きで提案します。推定効果（km/L / % / 年間節約）付き。
---
  ...
  <div id="adviceResult" class="d-none">
    ...
    <div class="card">
      <div class="card-body">
        ...
        <p class="small text-muted mt-2">このツールは経験則と公開データにもとづく簡易推定です。精密な判定には OBD のLTFT確認や満タン法をおすすめします。</p>
      </div>
    </div>
  </div>
</div>
...
const ACTIONS = [
  ...
  {
    id: 'injector',
    name: '燃料系（インジェクタ・燃焼室）クリーニング（添加剤 / プロ清掃）',
    cost_jpy: 3000,
    max_pct: 0.08,
  ...
  {
    id: 'ecu',
    name: 'ECU学習/診断・タイミング調整',
    cost_jpy: 0,
    max_pct: 0.03,
    desc: 'ECUの学習が改善されると若干の燃費向上。OBD リセットや学習ドライブが有効。'
  },
  {
    id: 'spark',
    ...
    cost_jpy: 6000,
    max_pct: 0.03,
    desc: 'プラグの劣化は燃焼不良の原因。古い車程効果が出やすい。'
  }
];
    ...
    card.innerHTML = `
      <div class="card-body">
        ...
        <div>
          ...
          <ul class="small mb-0">
            ${it.id === 'oil' ? '<li>オイルとオイルフィルタを交換（指定粘度、メーカー推奨）。</li>' : ''}
            ${it.id === 'tyre_pressure' ? '<li>各タイヤを規定空気圧に調整。月1回点検を推奨。</li>' : ''}
            ${it.id === 'tyre_replace' ? '<li>低転がり抵抗の新品タイヤへ交換。サイズ/等級を確認。</li>' : ''}
            ${it.id === 'injector' ? '<li>市販の燃料系クリーナーを1タンク分投入、もしくは洗浄施工を実施。</li>' : ''}
            ${it.id === 'driving' ? '<li>発進は穏やかに：0→20km/hをゆっくり、定速巡航で燃費改善。</li>' : ''}
            ${it.id === 'ecu' ? '<li>OBDで長短期燃料補正を確認し、必要であれば学習ドライブ（長時間の一定速度巡航）を行う。</li>' : ''}
